...
def main(cfg):
    ...
    trainer.test(model = mymodel, \
                 dataloaders = DataLoader(HDF5Dataset(cfg.dataset_path, n_frames=(checkpoint_params.cfg.num_frames if 'num_frames' in checkpoint_params.cfg else 2)), batch_size=1, shuffle=False))
    wandb.finish()
