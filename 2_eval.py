...
def main(cfg):
    ...
    trainer.validate(model = mymodel, \
                     dataloaders = DataLoader(HDF5Dataset(cfg.dataset_path + f"/{cfg.av2_mode}", n_frames=(checkpoint_params.cfg.num_frames if 'num_frames' in checkpoint_params.cfg else 2), eval=True, leaderboard_version=cfg.leaderboard_version), batch_size=1, shuffle=False))
    wandb.finish()
